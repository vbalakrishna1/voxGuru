...
class BookLiveScreen extends Component {
  ...
  componentDidMount() {
    ...
    console.log('-----navi id', this.props)

    firebase.firestore().collection("users").doc(userId).collection("LessonStatus").get().then(function (doc) {
      // console.log('---info', doc.data())
      console.log('---info', doc._docs.length)
      if (doc._docs.length <= 0) {
        lessonStatus = false
      } else {
        lessonStatus = true
      }
          ...
          firebase.firestore().collection("users").doc(userId).get().then(function (doc) {
            if (doc.exists) {
              ...
              temp["info"] = {
                ...
                mobile_number: steps.phone.value,
                genderText: steps.gender.value,
        ...
        <View style={{ flex: 1 }}>
          <ChatBot
            ...
            steps={[
              ...
              {
                id: '8',
                message: 'And your whatsapp number please?',
                trigger: 'phone',
              },
              {
                id: 'phone',
